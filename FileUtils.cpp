...
namespace FileUtils {
  ...
  int chrStrToInt(const string &chrom, int Nauto) {
    if (isdigit(chrom[0])) {
  ...
  int checkBgenSample(const std::string &bgenFile, const std::string &sampleFile, int Nautosomes) {
    ...
    uint16_t LS; fread(&LS, 2, 1, fin); // cout << "LS: " << LS << endl;
    fread(snpID, 1, LS, fin); snpID[LS] = '\0'; cout << "first snpID: " << string(snpID) << endl;
    //fseek(fin, LS, SEEK_CUR); // skip SNP id
    uint16_t LR; fread(&LR, 2, 1, fin); // cout << "LR: " << LR << endl;
    fread(rsID, 1, LR, fin); rsID[LR] = '\0'; cout << "first rsID: " << string(rsID) << endl;
    ...
    if (Layout==2) {
      ...
      int chrom = chrStrToInt(chrStr, Nautosomes);
      if (chrom == -1) {
	...
	exit(1);
      ...
      uint16_t Kheader; fread(&Kheader, 2, 1, fin); //cout << "K: " << K << endl;
      if (Kheader != 2) {

...
//double *sigma2Kbest, double *logDeltaBest, double *bestAbsF,
  ...
  vector <double> Bolt::reEstLogDeltas(const vector <double> &pheno, double logDeltaEst,
    ...
    for (uint64 b = 0; b < B; b++) {
      ...
      if (isnan(logDeltaReEsts[b]) || logDeltaReEsts[b] < logDeltaEst) {
  ...
  string Bolt::getSnpStats(const string &ID, int chrom,
    ...
    for (uint s = 0; s < retroData.size(); s++) {
      ...
      double dotProd = NumericUtils::dot(workVec, &calibratedResidsChunk[0], Nstride);
      double stat = BAD_SNP_STAT; double pValue = 1;
      if (!(projNorm2 < 0.1)) {
	...
	double beta = stat / xPerAlleleVinvPhi;
  ...
  (const string &outFile, const vector <string> &dosageFiles, const string &dosageFidIidFile,
   ...
   const vector <StatsDataRetroLOCO> &retroData) const {
    ...
    MapInterpolater mapInterpolater(geneticMapFile); // ok in no map; then always returns 0
	...
	if (!iss) {
	  cerr << "ERROR: Two few fields in line " << lineNum << " of " << dosageFiles[f] << endl;
	  exit(1);
	}
	if (iss >> line) {
	  cerr << "ERROR: Two many fields in line " << lineNum << " of " << dosageFiles[f] << endl;
	  exit(1);
	}
  ...
  (const string &outFile, const vector <string> &dosage2MapFiles,
   ...
   const vector <StatsDataRetroLOCO> &retroData) const {
	...
	if (!issGenoLine) {
	  cerr << "ERROR: Two few fields in line " << lineNum << " of " << dosage2GenoFiles[f]
	       << endl;
	  exit(1);
	}
	if (issGenoLine >> genoLine) {
	  cerr << "ERROR: Two many fields in line " << lineNum << " of " << dosage2GenoFiles[f]
	       << endl;
	  exit(1);
	}
  ...
  (const string &outFile, const vector <string> &impute2Files, const vector <int> &impute2Chroms,
   ...
   bool verboseStats, const vector <StatsDataRetroLOCO> &retroData) const {
	...
	if (!iss) {
	  cerr << "ERROR: Two few fields in line " << lineNum << " of " << impute2Files[f] << endl;
	  exit(1);
	}
	if (iss >> line) {
	  cerr << "ERROR: Two many fields in line " << lineNum << " of " << impute2Files[f]
	       << endl;
	  exit(1);
	}
...
#pragma omp parallel for
  ...
  (const string &outFile, int f, const string &bgenFile, const string &sampleFile,
   ...
   const vector <StatsDataRetroLOCO> &retroData, bool domRecHetTest)
    ...
    uint H; fread(&H, 4, 1, fin); // cout << "H: " << H << endl;
    uint Mbgen; fread(&Mbgen, 4, 1, fin); cout << "BGEN snpBlocks (Mbgen): " << Mbgen << endl;
    uint Nbgen; fread(&Nbgen, 4, 1, fin); cout << "BGEN samples (Nbgen): " << Nbgen << endl;
    if (Nbgen != Nsample) {
    ...
    uint16_t *shortBuf = (uint16_t *) ALIGNED_MALLOC_UCHARS(6*Nbgen);
	...
	exit(1);
      ...
      uint16_t LS; fread(&LS, 2, 1, fin); // cout << "LS: " << LS << " " << std::flush;
      fread(snpID, 1, LS, fin); snpID[LS] = '\0'; // cout << "snpID: " << string(snpID) << " " << std::flush;
      uint16_t LR; fread(&LR, 2, 1, fin); // cout << "LR: " << LR << " " << std::flush;
      fread(rsID, 1, LR, fin); rsID[LR] = '\0'; // cout << "rsID: " << string(rsID) << " " << std::flush;
      uint16_t LC; fread(&LC, 2, 1, fin); // cout << "LC: " << LC << " " << std::flush;
  ...
  string Bolt::getSnpStatsBgen2(uchar *buf, uint bufLen, const uchar *zBuf, uint zBufLen,
    ...
    if (maf < bgenMinMAF || info < bgenMinINFO) return "";
  ...
  (const string &outFile, int f, const string &bgenFile, const string &sampleFile,
   ...
   const vector <StatsDataRetroLOCO> &retroData, bool domRecHetTest, int threads)
	...
	exit(1);
      ...
      uint16_t K; fread(&K, 2, 1, fin); //cout << "K: " << K << endl;
      if (K != 2) {
